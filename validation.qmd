...
```{r}
#| label: discrimination-analysis-fixed
#| echo: false
#| warning: false
#| message: false
#| fig-width: 8
...
# ROC curve for maximum score predicting CRE infection
roc_obj <- roc(score_cre_analysis$cre_infection, 
               score_cre_analysis$max_score,
               quiet = TRUE)

# Calculate AUC
auc_value <- auc(roc_obj)
ci_auc_vals <- ci.auc(roc_obj)

cat("=== ROC Analysis Results ===\n")
cat("AUC for max score:", round(auc_value, 3), "\n")
cat("95% CI:", round(ci_auc_vals[1], 3), "-", round(ci_auc_vals[3], 3), "\n\n")

# Find optimal cutpoint
coords_best <- coords(roc_obj, "best", method = "youden")
cat("Optimal cutpoint (Youden method):\n")
cat("  Score threshold:", round(coords_best$threshold, 1), "\n")
cat("  Sensitivity:", round(coords_best$sensitivity * 100, 1), "%\n")
cat("  Specificity:", round(coords_best$specificity * 100, 1), "%\n\n")

# Create the plot without using abline
ggroc(roc_obj, legacy.axes = TRUE) +
  geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1), 
               color = "gray", linetype = "dashed") +
  geom_point(x = 1 - coords_best$specificity, 
             y = coords_best$sensitivity,
             color = "red", size = 4) +
  annotate("text", 
           x = 0.6, y = 0.3, 
           ...
           size = 5) +
  annotate("text",
           x = 1 - coords_best$specificity + 0.05,
           y = coords_best$sensitivity - 0.05,
           label = paste("Cutoff =", round(coords_best$threshold, 1)),
           color = "red", size = 4) +
  labs(
    title = "ROC Curve for CRE Risk Score",
...
# Prepare data for decision curve
dca_data <- score_cre_analysis %>%
  mutate(
    cre_prob = max_score / 100,  # Convert score to probability-like scale
    cre_outcome = cre_infection
  )

# Simple decision curve
decision_curve(cre_outcome ~ cre_prob, 
               data = dca_data,
...
score_velocity <- longitudinal_data %>%
  group_by(record_id) %>%
  arrange(days_from_olt) %>%
  mutate(
    score_change = score - lag(score),
    days_between = days_from_olt - lag(days_from_olt),
    velocity = score_change / days_between,
    acceleration = velocity - lag(velocity)
  ) %>%
  filter(!is.na(velocity))

# Summarize velocity by outcome
velocity_summary <- score_velocity %>%
...
# Compare velocities
velocity_comparison <- velocity_summary %>%
  group_by(primary_outcome) %>%
  summarise(
    n = n(),
    avg_velocity = mean(mean_velocity, na.rm = TRUE),
    avg_max_velocity = mean(max_velocity, na.rm = TRUE),
    .groups = "drop"
  )
...
risk_performance <- score_cre_analysis %>%
  mutate(
    risk_category = cut(max_score,
  ...
  summarise(
    ...
    mortality_rate = mean(primary_outcome == "Death", na.rm = TRUE),
    .groups = "drop"
  )

print(risk_performance)

# Calculate NNT for each category
risk_performance <- risk_performance %>%
  mutate(
    nnt = round(1 / cre_rate),
    recommendation = case_when(
      risk_category == "Low" ~ "Standard monitoring",
      risk_category == "Moderate" ~ "Enhanced surveillance",
      risk_category == "High" ~ "Consider prophylaxis",
      risk_category == "Very High" ~ "Aggressive intervention"
    )
  )
